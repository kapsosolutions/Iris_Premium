...
async function publishCategoryFlow() {
  try {
    ...
    // 2. Load Category Flow JSON
    const flowJsonPath = path.join(process.cwd(), 'flows', 'iris_category_flow.json');
    ...
    console.log(`\nUploading Category Flow JSON asset to Meta Graph API for Flow ID: ${FLOW_ID}...`);
    ...
    console.log(`\nPublishing Flow ID: ${FLOW_ID} in Meta WABA...`);
    const pubRes = await axios.post(`https://graph.facebook.com/v21.0/${FLOW_ID}/publish`, {}, {
      headers: { 'Authorization': `Bearer ${TOKEN}` }
    });

    console.log('🎉 SUCCESS! Dedicated Category Flow Published Live in Meta WABA!', pubRes.data);
    process.exit(0);
